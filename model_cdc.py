...
############RCL
            dist = torch.zeros(n_sample, 2 * n_sample - 1).cuda()
            # iterating over different elements in the batch
            for pair1 in range(n_sample):
                pi_feat = torch.unsqueeze(pre_out[pair1 + n_sample], 0)
                zi_feat = torch.unsqueeze(pre_out[pair1], 0)
                dist[pair1, pair1] = (1 - self.sim(pi_feat, zi_feat)) / 0.07
                t = 4
                for pair2 in range(n_sample):
                    if  pair1 != pair2:
                        zj_feat = torch.unsqueeze(pre_out[pair2], 0)
                        dist[pair1, pair2] = (1 - self.sim(pi_feat, zj_feat)) / 0.07
                        pj_feat = torch.unsqueeze(pre_out[pair2 + n_sample], 0)
                        dist[pair1, t] = (1 - self.sim(pj_feat, zi_feat)) / 0.07
                        t += 1
            dist = F.log_softmax(dist, dim=1)
            sims = -torch.diagonal(dist).mean()
