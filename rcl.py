...
def generate(args, g_ema, device, mean_latent, n_ckpt, batch_size=8, n_samples=5000):
    with torch.no_grad():
        g_ema.eval()
        save_path = os.path.join(args.root, f"checkpoint/10-sketches5_RCL/individual/{n_ckpt}")
...
def train(args, loader, generator, discriminator, extra, g_optim, d_optim, e_optim, g_ema, device):
    ...
    for idx in pbar:
        ...
        if which == 0:
            ...
            _, sim_pred = discriminator(inp_imgs, extra=extra, flag=which, p_ind=numpy.random.randint(lb, ub), sim=True)
            rel_loss =  sim_pred
        ...
        if which == 0:
            ...
            # #########a new contrastive loss
            # dist_target = torch.zeros([args.batch, args.batch]).cuda()
            # dist_source = torch.zeros([args.batch, args.batch]).cuda()
            #
            # # iterating over different elements in the batch
            # for pair1 in range(args.batch):
            #     pi_feat = torch.unsqueeze(feat_target[feat_ind[pair1]][pair1].reshape(-1), 0)
            #     zi_feat = torch.unsqueeze(feat_source[feat_ind[pair1]][pair1].reshape(-1), 0)
            #     dist_target[pair1, pair1] = sim(pi_feat, zi_feat) / args.temp
            #     dist_source[pair1, pair1] = dist_target[pair1, pair1]
            #     for pair2 in range(args.batch):
            #         if pair1 != pair2:
            #             zj_feat = torch.unsqueeze(feat_source[feat_ind[pair1]][pair2].reshape(-1), 0)
            #             pj_feat = torch.unsqueeze(feat_source[feat_ind[pair1]][pair2].reshape(-1), 0)
            #             dist_target[pair1, pair2] = sim(pi_feat, zj_feat)/args.temp
            #             dist_source[pair1, pair2] = sim(pj_feat, zi_feat) / args.temp
            # dist_target = sfm(dist_target)
            # dist_source = sfm(dist_source)
            # g_target = torch.diagonal(1 - dist_target)
            # g_source = torch.diagonal(1 - dist_source)
            # cl_g = -torch.log(g_target).mean() - -torch.log(g_source).mean()

            #########a new contrastive loss

            dist = torch.zeros([args.batch, 2 * args.batch - 1]).cuda()
            # iterating over different elements in the batch
            for pair1 in range(args.batch):
                pi_feat = torch.unsqueeze(feat_target[feat_ind[pair1]][pair1].reshape(-1), 0)
                zi_feat = torch.unsqueeze(feat_source[feat_ind[pair1]][pair1].reshape(-1), 0)
                dist[pair1, pair1] = (1 - sim(pi_feat, zi_feat)) / args.temp
                t = 4
                for pair2 in range(args.batch):
                    if pair1 != pair2:
                        zj_feat = torch.unsqueeze(feat_source[feat_ind[pair1]][pair2].reshape(-1), 0)
                        dist[pair1, pair2] = (1 - sim(pi_feat, zj_feat)) / args.temp
                        pj_feat = torch.unsqueeze(feat_target[feat_ind[pair1]][pair2].reshape(-1), 0)
                        dist[pair1, t] = (1 - sim(pj_feat, zi_feat)) / args.temp
                        t += 1
            dist = F.log_softmax(dist, dim=1)
            cl_g = -torch.diagonal(dist).mean()


            g_loss = g_loss + 1 * cl_g
        ...
        if get_rank() == 0:
            ...
            if i % args.sample_every == 0:
                with torch.no_grad():
                    ...
                    utils.save_image(
                        sample,
                        args.root+f"/checkpoint/10-sketches5_RCL/{str(i).zfill(6)}.png",
                    ...
                    utils.save_image(
                        interp,
                        args.root + f"/checkpoint/10-sketches5_RCL/interp_{str(i).zfill(6)}.png",
                        nrow=int(8),
                        normalize=True,
                        range=(-1, 1),
                    )
            ...
            if i > 0 and i % args.save_every == 0:
                ...
                torch.save(
                    ...
                    args.root + f"/checkpoint/10-sketches5_RCL/{str(i).zfill(6)}.pt",
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--iter", type=int, default=70001, help="total training iterations")
    ...
    parser.add_argument("--save_every", default=5000, type=int, help="when to save model checkpoints")
    ...
    parser.add_argument("--temp", type=int, default=0.07)
